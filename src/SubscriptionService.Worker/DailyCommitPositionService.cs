...
public sealed class DailyCommitPositionService : BackgroundService
{
    ...
    private readonly DailyCommitPositionPlanner _planner;
    private readonly TimeSpan _interval = TimeSpan.FromHours(24);

    public DailyCommitPositionService(IDailyCommitPositionStore store, ISubscriptionConfigurationStore configStore, ISubscriptionEventSource eventSource, IIndexScanStateStore scanStateStore, DailyCommitPositionPlanner planner)
    {
        this._store = store;
        this._configStore = configStore;
        this._eventSource = eventSource;
        this._scanStateStore = scanStateStore;
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                var subs = await this._configStore.GetSubscriptionsAsync(stoppingToken);
                ...
                foreach (var kvp in indexToSubs)
                {
                    ...
                    tasks.Add(Task.Run(async () =>
                    {
                        ...
                        try
                        {
                            // Skip only when every subscription for this index already has today's row.
                            // A shared index must continue scanning when any subscriber is missing it.
                            var subscribersNeedingRows = await this._planner.GetSubscriptionsMissingDateAsync(subscribers, today, stoppingToken);
                            if (subscribersNeedingRows.Count == 0) return;

                            // Resume scan from last scanned commit position to avoid starting from beginning on restart
                            var lastScanned = await this._scanStateStore.GetLastScannedCommitPositionAsync(index, stoppingToken);
                            ...
                            while (!stoppingToken.IsCancellationRequested && consecutiveEmpty < maxEmptyWindows)
                            {
                                ...
                                try
                                {
                                    ...
                                    await this._eventSource.SubscribeAsync(subscribers[0], checkpointState, async (@event, ct) =>
                                    {
                                        foundAny = true;
                                        ...
                                        {
                                            var existingRow = await this._store.GetCommitPositionForDateAsync(s.SubscriptionId, index, eventDate, ct);
                                            if (existingRow is null)
                                            {
                                                await this._store.UpsertAsync(s.SubscriptionId, index, eventDate, commit, ct);
                                            }
                                        }
                                        ...
                                        if (commit is not null)
                                        {
                                            ...
                                            await this._scanStateStore.SetLastScannedCommitPositionAsync(index, scanLastScanned, ct);
                                        }
